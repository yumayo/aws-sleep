...
`accessKeyId` / `secretAccessKey` / `sessionToken` は保存時に暗号化されるため、静的キーを使う場合は管理画面から入力してください。
...
```json
{
  "awsAccounts": [
    ...
    },
    {
      ...
      "accountName": "本番アカウント",
      "awsRegion": "ap-northeast-1",
      "credentialProfile": "aws-sleep-prod"
    },
    {
...
管理画面で `accessKeyId` / `secretAccessKey` を指定すると静的認証情報を使用します。一時認証情報の場合は `sessionToken` も指定できます。これらの認証情報は `AWS_SLEEP_CONFIG_ENCRYPTION_KEY` で暗号化され、`config.json` には平文で保存されません。
...
AWS_SLEEP_CONFIG_ENCRYPTION_KEY=
```

`AWS_SLEEP_CONFIG_ENCRYPTION_KEY` は管理画面から入力したAWS認証情報を暗号化・復号するためのキーです。32文字以上のランダム値を設定し、値を変更すると保存済みの静的キーを復号できなくなるため、運用中は固定してください。
